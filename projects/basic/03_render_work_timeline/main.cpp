...
struct PerFrameData
{
  ...
  uint64_t                work_complete_label     = 0;
};
...
public:
  VkexInfoApp()
    : vkex::Application(k_window_width, k_window_height, "03_render_work_timeline")
  {
  }
  virtual ~VkexInfoApp()
  {
...
void VkexInfoApp::Render(Application::RenderData* p_current_render_data, Application::PresentData* p_current_present_data)
{
  ...
  VkResult vk_result = per_frame_data.work_complete_semaphore->Wait(per_frame_data.work_complete_label);
  VKEX_ASSERT(vk_result == VK_SUCCESS);
