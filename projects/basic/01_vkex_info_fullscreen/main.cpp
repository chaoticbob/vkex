...
public:
  VkexInfoApp() : vkex::Application(k_window_width, k_window_height, "01_vkex_info_fullscreen") {}
  virtual ~VkexInfoApp() {}
...
void VkexInfoApp::Setup()
{
  ...
  // Shader program
  {
    auto vs = asset_util::LoadFile(GetAssetPath("shaders/DrawTexture.vs.spv"));
    VKEX_ASSERT_MSG(!vs.empty(), "Vertex shader failed to load!");
    auto ps = asset_util::LoadFile(GetAssetPath("shaders/DrawTexture.ps.spv"));
  ...
  // Pipeline
  {
    vkex::VertexBindingDescription vertex_binding_descriptions = p_vertex_buffer_cpu->GetVertexBindingDescription();
