...
const VkDescriptorSetLayoutBinding* CDescriptorSet::FindDescriptorBinding(uint32_t binding) const
{
    ...
    auto it = std::find_if(
        ...
        [binding](const VkDescriptorSetLayoutBinding& elem) -> bool { return elem.binding == binding; });
    ...
    const VkAllocationCallbacks*          p_allocator)
{
    ...
    if (m_create_info.max_sets == 0) {
        m_create_info.max_sets = m_create_info.pool_sizes.sampler + m_create_info.pool_sizes.combined_image_sampler + m_create_info.pool_sizes.sampled_image + m_create_info.pool_sizes.storage_image + m_create_info.pool_sizes.uniform_texel_buffer + m_create_info.pool_sizes.storage_texel_buffer + m_create_info.pool_sizes.uniform_buffer + m_create_info.pool_sizes.storage_buffer + m_create_info.pool_sizes.uniform_buffer_dynamic + m_create_info.pool_sizes.storage_buffer_dynamic + m_create_info.pool_sizes.input_attachment;
    }
