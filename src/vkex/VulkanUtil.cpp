...
void ShaderArguments::SortSets()
{
    std::sort(
        ...
        [](const AssignedDescriptorSet& a, AssignedDescriptorSet& b) { return a.set_number < b.set_number; });
}
...
vkex::Result ShaderArguments::AssignSet(uint32_t set_number, vkex::DescriptorSet descriptor_set)
{
    auto it = std::find_if(
        std::begin(m_assigned_sets),
        std::end(m_assigned_sets),
        [set_number](const AssignedDescriptorSet& elem) -> bool { return elem.set_number == set_number; });
...
bool IsMemoryHostVisible(VmaMemoryUsage usage)
{
    bool is_host_visible = (usage == VMA_MEMORY_USAGE_CPU_ONLY) ||
                           (usage == VMA_MEMORY_USAGE_CPU_TO_GPU) ||
                           (usage == VMA_MEMORY_USAGE_GPU_TO_CPU);
    return is_host_visible;
}

/** @fn IsMemoryUsageDeviceLocal
 *
 */
bool IsMemoryDeviceLocal(VmaMemoryUsage usage)
{
    bool is_device_local = (usage == VMA_MEMORY_USAGE_GPU_ONLY);
    return is_device_local;
}
