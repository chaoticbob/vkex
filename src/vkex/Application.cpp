...
vkex::Result Application::InitializeWindow()
{
    ...
    if (IsApplicationModeWindow()) {
        ...
        // Resize
        glfwWindowHint(GLFW_RESIZABLE, m_configuration.window.resizeable ? true : false);
...
#    endif
    ...
    // Image count
    {
        ...
        if (image_count != m_configuration.swapchain.image_count) {
            if (m_configuration.swapchain.image_count != 0) {
                VKEX_LOG_WARN("Swapchain image count readjusted from " << m_configuration.swapchain.image_count << " to " << image_count);
            }
    ...
    // Extents
    {
        ...
        auto& vk_surface_capabilities          = m_surface->GetVkSurfaceCapabilities2();
        {
            ...
            if ((max_width < m_configuration.window.width) || (max_height < m_configuration.window.height)) {
                VKEX_LOG_WARN("Swapchain extents readjusted using maxImageExtent from " << m_configuration.window.width << "x" << m_configuration.window.height << " to " << max_width << "x" << max_height);
...

        {
            ...
            if ((min_width > m_configuration.window.width) || (min_height > m_configuration.window.height)) {
                VKEX_LOG_WARN("Swapchain extents readjusted using minImageExtent from " << m_configuration.window.width << "x" << m_configuration.window.height << " to " << min_width << "x" << min_height);
                requested_window_extents_changed = true;
            }
    ...
    // Transition swapchain depth/stencil images to VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL
    {
        ...
        for (uint32_t image_index = 0; image_index < image_count; ++image_index) {
            ...
            VKEX_CALL(vkex::TransitionImageLayout(m_graphics_queue, image, image->GetInitialLayout(), VK_IMAGE_LAYOUT_PRESENT_SRC_KHR, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT));
            // Depth/stencil
            if (m_swapchain->HasDepthStencil()) {
                // Get depth/stencil image
                vkex::Result vkex_result = vkex::Result::Undefined;
                VKEX_RESULT_CALL(
                    vkex_result,
                ...
                // Transition
                vkex_result = vkex::Result::Undefined;
                VKEX_CALL(vkex::TransitionImageLayout(m_graphics_queue, image, image->GetInitialLayout(), VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_PIPELINE_STAGE_VERTEX_SHADER_BIT));
            }
        }
    }
    ...
    // Log swapchain creation
    {
        ...
        VKEX_LOG_INFO("   "
                      << "Present Mode     : " << vkex::ToString(m_configuration.swapchain.present_mode));
        VKEX_LOG_INFO("   "
    ...
    if (m_screen_shot && m_configuration.enable_screen_shot) {
        ...
        vkex::CommandBuffer command_buffer = p_present_data->GetCommandBuffer();
        {
            ...
            command_buffer->CmdTransitionImageLayout(
                *image, image->GetAspectFlags(), 0, image->GetMipLevels(), 0, image->GetArrayLayers(), VK_IMAGE_LAYOUT_PRESENT_SRC_KHR, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, VK_PIPELINE_STAGE_TRANSFER_BIT);
            // Copy
            ...
            // Transition image
            command_buffer->CmdTransitionImageLayout(
                *image, image->GetAspectFlags(), 0, image->GetMipLevels(), 0, image->GetArrayLayers(), VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, VK_IMAGE_LAYOUT_PRESENT_SRC_KHR, VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT);
            command_buffer->End();
        }
