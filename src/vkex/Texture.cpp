    ...
    const VkAllocationCallbacks*   p_allocator)
{
    ...
    if (m_create_info.existing_image != nullptr) {
        ...
        m_create_info.image.memory_usage         = m_image->GetMemoryUsage();
        ...
        // Fill in defaults
        {
            ...
            //
            if (IsMemoryHostVisible(m_create_info.image.memory_usage)) {
        ...
        image_create_info.memory_usage          = m_create_info.image.memory_usage;
...
VkImageSubresourceLayers CTexture::GetSubresourceLayers(
    uint32_t mip_level,
    uint32_t array_layer_start,
    uint32_t array_layer_count) const
{
    return m_image->GetSubresourceLayers(mip_level, array_layer_start, array_layer_count);
}
...
{
    bool is_host_visible = IsMemoryHostVisible(m_create_info.image.memory_usage);
